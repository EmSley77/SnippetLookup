...
export default function UserInfo({ user, formatCreatedDate, snippets }) {
    return (
        <div className="flex flex-col gap-4 lg:grid-cols-2 p-4 bg-white shadow-2xl rounded-xl w-1/2">

            {/* User Info Section */}
            <div className="flex flex-col justify-between p-4 text-gray-800 bg-gray-100 rounded-xl shadow-lg">
            ...
            <div className="text-center bg-white p-4 rounded-xl shadow-md text-gray-800">
                <h1 className="text-lg font-medium">My Snippets</h1>
                <hr />
            </div>

            {/* List of Snippets */}
            <div className="grid grid-cols-2 gap-10 overflow-y-auto bg-gray-400 p-3 rounded-xl">
                {snippets && snippets.length > 0 ? snippets.map((snippet) => (
                    <div key={snippet.id} className="p-4 bg-gray-200 rounded-lg shadow-lg hover:shadow-xl transition-all">
                        <div className="flex flex-col mb-3 h-full justify-between">
                            ...
                            {/* Description */}
                            <p className="text-gray-800 text-m mb-5 mt-3">{snippet.description.toUpperCase()}</p>
                            ...
                            {/* Code block */}
                            <pre className="bg-gray-700 p-3 text-teal-300 rounded-xl mt-5 whitespace-pre-wrap break-words overflow-y-auto max-h-40">
                                {snippet.code}
                            </pre>

                            {/* Username */}
                            <div className="text-gray-800 text-s font-extrabold mt-3">@{snippet.username}</div>
                        </div>
                    </div>
                )) : <h1 className="text-gray-800 text-lg">No snippets created yet</h1>}
            </div>
        </div>
    )
};
