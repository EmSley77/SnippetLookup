                ...
                {snippets && snippets.length > 0 ? snippets.map((snippet) => (
                    <div key={snippet.id} className="p-4 bg-gray-800 rounded-lg shadow-lg hover:shadow-xl transition-all">
                        <div className="flex flex-col mb-3">
                            ...
                            {/* Code block */}
                            <pre className="bg-gray-700 p-3 text-teal-200 rounded-xl mt-5  whitespace-pre-wrap break-words overflow-y-auto max-h-40">
                                {snippet.code}
                            </pre>

                            {/* Username */}
                            <div className="text-gray-100 text-s font-extrabold mt-3">@{snippet.username}</div>
                        </div>
                    </div>
                )) : <h1 className="text-white text-lg">No snippets created yet</h1>}
            </div>
        </div>
    )
}
